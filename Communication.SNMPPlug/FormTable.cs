...
namespace FreeSCADA.Communication.SNMPPlug
{
    internal partial class FormTable : Form
    {
        ...
        public void RefreshTable()
        {
           ...
           /* while (true)
            {
                IProfileRegistry registry = Plugin.Container.Resolve<IProfileRegistry>();
                if (registry == null)
                {
                    break;
                }

                SNMPAgent prof = registry.DefaultProfile;
                IList<Variable> list = new List<Variable>();
                Thread.Sleep(Convert.ToInt32(textBoxRefresh.Text, CultureInfo.CurrentCulture) * 1000);
                int rows = Messenger.Walk(prof.VersionCode, prof.AgentIP, new OctetString(prof.GetCommunity), new ObjectIdentifier(definition.GetNumericalForm()), list, 1000, WalkMode.WithinSubtree);
